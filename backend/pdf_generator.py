...
def generate_ppsps_pdf(chantier, ppsps, output_path):
    ...
    lines = [f"Responsable : {ppsps.responsable_chantier or ''}", f"Effectif : {ppsps.nb_compagnons} pers. | Horaires : {ppsps.horaires}", f"CSPS : {ppsps.coordonnateur_sps or ''} | MOA : {ppsps.maitre_ouvrage or ''}"]
    ...
    for t in taches:
        ...
        c.setFont(FONT_TITLE, 10); c.drawString(margin, y, t.get('tache','')); y -= 0.5*cm
        c.setFont(FONT_TEXT, 9); c.setFillColorRGB(0.8, 0, 0)
        c.drawString(margin+0.5*cm, y, f"Risque : {t.get('risque','')}"); y -= 0.5*cm
        c.setFillColorRGB(0, 0.4, 0)
        c.drawString(margin+0.5*cm, y, f"Prévention : {t.get('prevention','')}"); c.setFillColorRGB(0,0,0); y -= 0.8*cm
